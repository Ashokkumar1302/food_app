...
const createCatController = async (req, res) => {
    try {
      ...
      const newCategory = new categeoryModel({ title, imageUrl });
  ...
  const getAllCatController = async (req, res) => {
    try {
      const categories = await categeoryModel.find({});
      if (!categories) {
...
const updateCatController = async (req, res) => {
    try {
      const { id } = req.params;
      const { title, imageUrl } = req.body;
      const updatedCategory = await categeoryModel.findByIdAndUpdate(
        id,
        { title, imageUrl },
        { new: true }
      );
  ...
  const deleteCatController = async (req, res) => {
    try {
      ...
      const category = await categeoryModel.findById(id);
      if (!category) {
        return res.status(500).send({
          success: false,
      ...
      await categeoryModel.findByIdAndDelete(id);
      res.status(200).send({
        success: true,
